...
DiminutiveCoinGUI::DiminutiveCoinGUI(QWidget *parent):
    QMainWindow(parent),
    clientModel(0),
    walletModel(0),
    toolbar(0),
...
{
    resize(850+95, 550);
    setWindowTitle(tr("DiminutiveCoin") + " - " + tr("Wallet"));
#ifndef Q_OS_MAC
    qApp->setWindowIcon(QIcon(":icons/diminutivecoin"));
    setWindowIcon(QIcon(":icons/diminutivecoin"));
#else
...
DiminutiveCoinGUI::~DiminutiveCoinGUI()
{
...
void DiminutiveCoinGUI::createActions()
{
    ...
    sendCoinsAction->setToolTip(tr("Send coins to a DiminutiveCoin address"));
    ...
    aboutAction = new QAction(tr("&About DiminutiveCoin"), this);
    aboutAction->setToolTip(tr("Show information about DiminutiveCoin"));
    aboutAction->setMenuRole(QAction::AboutRole);
    aboutQtAction = new QAction(tr("About &Qt"), this);
    aboutQtAction->setToolTip(tr("Show information about Qt"));
    aboutQtAction->setMenuRole(QAction::AboutQtRole);
    optionsAction = new QAction(tr("&Options..."), this);
    optionsAction->setToolTip(tr("Modify configuration options for DiminutiveCoin"));
    optionsAction->setMenuRole(QAction::PreferencesRole);
    toggleHideAction = new QAction(QIcon(":/icons/diminutivecoin"), tr("&Show / Hide"), this);
...
void DiminutiveCoinGUI::createMenuBar()
{
...
void DiminutiveCoinGUI::createToolBars()
{
...
void DiminutiveCoinGUI::setClientModel(ClientModel *clientModel)
{
...
#ifndef Q_OS_MAC
            qApp->setWindowIcon(QIcon(":icons/diminutivecoin_testnet"));
            setWindowIcon(QIcon(":icons/diminutivecoin_testnet"));
#else
            MacDockIconHandler::instance()->setIcon(QIcon(":icons/diminutivecoin_testnet"));
#endif
            if(trayIcon)
            {
                trayIcon->setToolTip(tr("DiminutiveCoin client") + QString(" ") + tr("[testnet]"));
...
void DiminutiveCoinGUI::setWalletModel(WalletModel *walletModel)
{
...
void DiminutiveCoinGUI::createTrayIcon()
{
    QMenu *trayIconMenu;
#ifndef Q_OS_MAC
    ...
    trayIcon->setToolTip(tr("DiminutiveCoin client"));
...
#ifndef Q_OS_MAC
void DiminutiveCoinGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
void DiminutiveCoinGUI::optionsClicked()
{
...
void DiminutiveCoinGUI::aboutClicked()
{
    AboutDialog dlg;
...
void DiminutiveCoinGUI::setNumConnections(int count)
{
    QString icon;
    switch(count)
    {
    ...
    labelConnectionsIcon->setToolTip(tr("%n active connection(s) to DiminutiveCoin network", "", count));
}

void DiminutiveCoinGUI::setNumBlocks(int count)
{
...
void DiminutiveCoinGUI::message(const QString &title, const QString &message, bool modal, unsigned int style)
{
    QString strTitle = tr("DiminutiveCoin") + " - ";
...
void DiminutiveCoinGUI::changeEvent(QEvent *e)
{
...
void DiminutiveCoinGUI::closeEvent(QCloseEvent *event)
{
    if(clientModel)
    {
#ifndef Q_OS_MAC // Ignored on Mac
...
void DiminutiveCoinGUI::askFee(qint64 nFeeRequired, bool *payFee)
{
    if (!clientModel || !clientModel->getOptionsModel())
        return;
    ...
    QString strMessage = tr("This transaction is over the size limit. You can still send it for a fee of %1, "
        ...
        "Do you want to pay the fee?").arg(DiminutiveCoinUnits::formatWithUnit(clientModel->getOptionsModel()->getDisplayUnit(), nFeeRequired));
...
void DiminutiveCoinGUI::incomingTransaction(const QModelIndex & parent, int start, int end)
{
        ...
        notificator->notify(Notificator::Information,
                            (amount)<0 ? tr("Sent transaction") :
                              ...
                              tr("Date: %1\n"
                                 ...
                                 "Address: %4\n")
                              .arg(date)
                              .arg(DiminutiveCoinUnits::formatWithUnit(walletModel->getOptionsModel()->getDisplayUnit(), amount, true))
                              .arg(type)
                              .arg(address), icon);
    }
}

void DiminutiveCoinGUI::gotoOverviewPage()
{
...
void DiminutiveCoinGUI::gotoHistoryPage()
{
...
void DiminutiveCoinGUI::gotoAddressBookPage()
{
...
void DiminutiveCoinGUI::gotoBlockBrowser(QString transactionId)
{
...
void DiminutiveCoinGUI::gotoReceiveCoinsPage()
{
...
void DiminutiveCoinGUI::gotoSendCoinsPage()
{
...
void DiminutiveCoinGUI::gotoSignMessageTab(QString addr)
{
...
void DiminutiveCoinGUI::gotoVerifyMessageTab(QString addr)
{
...
void DiminutiveCoinGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void DiminutiveCoinGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
        ...
        if (nValidUrisFound)
            ...
            notificator->notify(Notificator::Warning, tr("URI handling"), tr("URI can not be parsed! This can be caused by an invalid DiminutiveCoin address or malformed URI parameters."));
    }

    event->acceptProposedAction();
}

void DiminutiveCoinGUI::handleURI(QString strURI)
{
    // URI has to be valid
...
void DiminutiveCoinGUI::setEncryptionStatus(int status)
{
    switch(status)
    {
...
void DiminutiveCoinGUI::encryptWallet()
{
    if(!walletModel)
        return;
...
void DiminutiveCoinGUI::backupWallet()
{
...
void DiminutiveCoinGUI::changePassphrase()
{
...
void DiminutiveCoinGUI::unlockWallet()
{
    if(!walletModel)
        return;
...
void DiminutiveCoinGUI::lockWallet()
{
    if(!walletModel)
        return;
...
void DiminutiveCoinGUI::showNormalIfMinimized(bool fToggleHidden)
{
...
void DiminutiveCoinGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void DiminutiveCoinGUI::updateWeight()
{
    if (!pwalletMain)
        return;
...
void DiminutiveCoinGUI::updateStakingIcon()
{
    updateWeight();
...
void DiminutiveCoinGUI::detectShutdown()
{
